...
class AbsTabletop() :
    ...
    def activatePlayer( self, iPlayer, aDataTreeAbs ):
        pass
    
    def sleepPlayer( self, iPlayer, aDataTreeAbs, result ):
        pass
...
class TabletopNet() :
    ...
    def wakeUpPlayers( self, gameConfiguration ):
        gc256= gameConfiguration.asDataTree()
    ...
    def activatePlayer( self, iPlayer, playerHand ):
        ...
        while True :
            ...
            if sockid == playerSockId :
                ...
                return DataTree().load(actionStr)
            else :
                self.socket.send_multipart( [sockid, b'', b'stop\nerror protocol'] )
...
class TabletopLocal() :
    ...
    def wakeUpPlayers( self, gameConfiguration ):
        ...
        for player in self.players[1:] :
            ...
            player.wakeUp( iPlayer, numberOfPlayers, DataTree().load(gameDump) )
            iPlayer+= 1
    ...
    def activatePlayer( self, iPlayer, playerHand ):
        ...
        self.players[iPlayer].perceive( DataTree().load( ph256.dump() ) )
...
class SeatClient() :
    ...
    def takeASeat(self, host='localhost', port=1400 ):
        ...
        while msg[0] != 'stop' :
            ...
            if msg[0] == 'perception' :
                self.player.perceive( DataTree().load( msg[1:] ) )
                ...
                self.player.wakeUp( 
                    int( playerMsg[1] ), int( playerMsg[3] ), DataTree().load( gameConfigurationMsg )
                )
                self.send( "ready" )
            elif msg[0] == 'sleep' :
                self.player.perceive( DataTree().load( msg[2:] ) )
